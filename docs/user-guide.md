...
- **配置**：在连接管理中新建或编辑连接时，勾选「只读连接」并保存。
- **效果**：该连接下，表数据视图仅可查看，不可编辑、导入、批量操作或开启事务；表右键「导入」不显示。若通过接口尝试写入，将提示「connection is read-only」。

### 5.2 审计日志

- **入口**：侧栏「审计日志」打开审计日志弹窗。
- **内容**：记录查询执行、表数据更新/删除/插入、表导入、导出、备份、恢复等操作；每条包含时间、操作类型、详情、连接/库/表（如有）。
- **查询与导出**：可按操作类型筛选，支持导出为 CSV 或 JSON 供离线查看。日志文件位于用户配置目录下的 `audit.jsonl`。
...
- **入口**：侧栏「数据对比」打开数据对比弹窗。
- **选择数据源**：分别选择源 A、源 B 的「连接」「库」「表」。
- **对比**：点击「对比」后，拉取两表前 1000 行，按行号逐行比对，并标注状态：**相同**、**已修改**、**仅 A**、**仅 B**。差异行高亮展示，修改的单元格显示 `A 值 → B 值`。
- **导出报告**：点击「导出报告」可下载 TSV 文件，含状态、行号及各列 A/B 值，便于归档或二次分析。

### 6.2 Schema 同步

- **入口**：侧栏「Schema 同步」打开 Schema 同步弹窗。
- **选择数据源**：同样选择源 A、源 B 的连接、库、表。
- **生成脚本**：
  - **生成 A→B**：生成使 B 与 A 表结构一致的 `ALTER TABLE` 等 SQL（新增列、修改列、可选的删除列建议，删除列以注释形式给出，需人工确认后执行）。
  - **生成 B→A**：生成使 A 与 B 表结构一致的同步脚本。
- **复制脚本**：在弹窗中查看生成的 SQL，点击「复制」到剪贴板，在目标库执行前请务必核对。仅支持 MySQL、PostgreSQL。

## 七、常见问题

**Q：数据对比或 Schema 同步时选不中库/表？**  
A：请确保对应连接已添加且可用；先选连接，再选库，最后选表，若库表列表为空，请检查连接配置与权限。
