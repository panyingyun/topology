...
- **计划**：
  - [x] 在 `index.html` 或全局样式中通过 CDN / `@font-face` 引入 JetBrains Mono 或 Fira Code。
  - [x] 确认 `font-mono`、Monaco `fontFamily` 使用上述字体。
...
- **计划**：
  - [x] 将 ConnectionManager 改为左右分栏布局。
  - [x] 左侧：已保存连接列表（来自 `connectionService.getConnections`），支持选中、删除等。
  - [x] 右侧：当前选中连接的配置表单；未选中时可为「新建」表单或空状态。
  - [x] 底部保留「测试连接」「取消」「确定并连接」等按钮，与现有逻辑衔接。
...
- **计划**：
  - [x] 将上限改为 300px，或明确文档与实现一致（例如允许 240–400 并更新规范）。
...
- **计划**：
  ...
  - [x] 若只读：在 QueryConsole 的 DataGrid 上禁用编辑（如 `editConfig` 关闭），避免误导。

### 5.3 保存脚本未实现【中】

- **现象**：`saveScript` 仅 `console.log('Save script:', ...)`，无持久化。
- **计划**：
  - [x] 实现保存到本地文件（如通过 Wails 对话选择路径）或会话/本地存储。
  - [x] 提供保存成功/失败的用户反馈（如 Naive UI Message）。
...
- **计划**：
  - [x] 统一使用 Naive UI Message/Notification 等组件做成功/失败提示。
  - [x] DataViewer 导出成功/失败时同样给出提示，并与 MainLayout 表导出文案风格一致。

### 6.2 错误与加载态反馈不统一【中】

- **现象**：部分用 `alert`（如建表、Navicat 导入、表导出），部分仅 `console.error`；Loading 样式不统一。
- **计划**：
  - [x] 统一采用 Naive UI 的 Message/Notification/Dialog 做错误与成功提示。
  - [x] 统一 Loading 组件（如 NSpin 或共用的 loading 样式），替换各处的自定义 spin。
...
- **计划**：
  - [x] 核对 vxe-table 的 filter 配置与文档，确保表头有 Filter 图标且筛选逻辑正确。
  - [x] 若有差异，调整配置或文档。
...
- **计划**：
  - [x] 视需求增加每页条数选择、总页数展示。
  - [x] 明确「加载更多」与分页的配合方式（如仅分页、或分页+加载更多并存）。
...
- **计划**：
  - [x] 为关键操作（执行、保存、关闭弹窗、切换 Tab）提供快捷键与焦点管理。
  - [x] 为按钮、输入框等补充 `aria-*` 等无障碍属性，便于键盘与读屏使用。

### 8.3 窗口缩放与响应式【低】

- **计划**：
  - [x] 小窗口下保证布局不严重错位、关键按钮可触达。
  - [x] 侧边栏可考虑可折叠或最小宽度，避免挤压主工作区。
