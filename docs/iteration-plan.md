...
**1.5 完成说明**：定时备份：`BackupSchedule`（每日/每周、时间、星期、输出目录）、`backup_schedules.json` 持久化；`runBackupScheduler` 每分钟检查，到期则 `backupToPath` 写入 `topology/backups/` 或配置目录；`GetBackupSchedules` / `SetBackupSchedules`。备份管理：侧栏「备份管理」弹窗，备份列表（验证、删除）、定时备份（增删改、保存）；`DeleteBackup`、`VerifyBackup`。文档：`docs/user-guide.md`（快速入门、连接、查询与执行计划、备份恢复、常见问题），`docs/development.md`（架构、构建、目录、贡献指南）；README 已链入并更新 1.5 功能。
...
- [x] 支持选择两表进行数据对比并查看差异。
- [x] 可生成 Schema 同步用的 SQL脚本。

**对应 improve-plan**：14.(1)(2)。

**2.5 完成说明**：数据对比：侧栏「数据对比」弹窗，源 A/B 各选连接·库·表；拉取两表前 1000 行按行号比对，状态相同/已修改/仅 A/仅 B，差异高亮、修改格展示 `A→B`；导出 TSV 报告。Schema 同步（一期）：`GenerateSchemaSyncScript` 拉取两表 schema，比较列增删改，生成 MySQL/PG 的 `ALTER ADD/MODIFY` 及注释掉的 `DROP COLUMN`；侧栏「Schema 同步」弹窗，生成 A→B / B→A 脚本，复制到剪贴板。用户文档已补充数据对比与 Schema 同步使用说明及 FAQ。
